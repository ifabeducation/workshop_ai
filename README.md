...
- **Step 4 — Use Case**: un unico step per il caso d'uso. Non c'è un modulo da compilare: l'agente conduce un'intervista che parte dalla domanda generica su com'è oggi il processo e qual è il problema, ed estrae i campi della scheda da quello che il partecipante racconta — anche più campi da una sola risposta. Il passaggio alla scheda (sola lettura, con export PDF) lo decide la conversazione stessa, non un'autorizzazione esterna. Vedi [Step 4 — Use Case: intervista e scheda](#step-4--use-case-intervista-e-scheda).
...
Lo Step 4 e la scheda Use Case sono **un unico step**: la scheda (che ricalca il template `Workshop1_Template_Use_Case_Submission_1_page.docx`) non si compila mai a mano campo per campo, si compila conversando, e resta **sola lettura** anche a intervista conclusa. Il facilitatore sblocca lo step quando il gruppo è pronto, ma il passaggio dall'intervista alla scheda non dipende da nessuna autorizzazione esterna: lo decide la conversazione stessa (vedi `src/lib/types.ts` → `Block2Submission.canProceedToUseCase`).

**Fase 1 — intervista.** L'agente (modello `USE_CASE_MODEL`, vedi [Modelli OpenAI](#modelli-openai)) parte dalla domanda generica dello Step 4 (com'è oggi il processo, chi è coinvolto, dove si inceppa, cosa costa) e da lì chiede solo quello che non ha ancora sentito, comportandosi da intervistatore e non da questionario. Le domande sono raggruppate per **argomento**: un argomento raccoglie tutti i campi che si possono ottenere con una domanda sola, così bastano ~11 domande per 28 campi. Una barra in alto mostra gli argomenti coperti.

- A ogni turno l'agente risponde in **JSON**: il messaggio per il partecipante, i campi che ha ricavato, gli argomenti che considera chiusi e (quando pertinente) i campi ormai segnalati come "non disponibili". Rilegge l'intera conversazione ad ogni turno — non solo l'ultimo messaggio — così una risposta lunga può chiudere più argomenti insieme, e non richiede mai un'informazione già emersa altrove. I campi passano da `sanitizeInterviewFields` (id inesistenti, valori vuoti e opzioni non previste vengono scartati): nella scheda non può finire un valore che il form non sa mostrare.
- **Gli argomenti ancora aperti li decide il server**, dai `closedGroups` accumulati nella submission, non il modello: un argomento si chiude solo quando ogni suo campo risulta compilato o esplicitamente segnalato non disponibile (validato in `api/agent/route.ts`, non sulla parola del modello).
- Risposte come "non lo so" non chiudono da sole un argomento: l'agente prova un paio di follow-up (esempio, domanda più semplice, riformulazione) prima di accettare che l'informazione non è disponibile.
- **Terminare prima che tutto sia completo.** Se il partecipante esprime l'intenzione di concludere ("ho finito", "non ho altro da aggiungere", equivalenti — riconosciuti per intenzione, non per corrispondenza testuale) mentre restano argomenti importanti mai affrontati, l'agente non si ferma: lo dice e continua con la domanda più utile. Se invece gli argomenti principali sono già coperti, chiede conferma esplicita ("abbiamo raccolto le informazioni che sei in grado di fornire... vuoi procedere comunque?", con due risposte rapide in chat); solo una conferma positiva autorizza il passaggio con campi ancora incompleti. Il server richiede comunque che sia già stato chiuso almeno un argomento prima di accettare sia la richiesta di conferma sia la conferma stessa: un "ho finito" scritto al primo messaggio non salta l'intervista.
- Quando `canProceedToUseCase` diventa vero (intervista naturalmente completa o conferma ottenuta) compare in fondo alla chat il pulsante **"Procedi allo Use Case"**. Il valore è persistito sulla submission: tornare all'intervista da una scheda già raggiunta e ripremere il pulsante riapre la stessa scheda senza rigenerarla né richiedere di nuovo la conferma.
...
**Fase 2 — scheda, sola lettura.** Si apre solo quando `canProceedToUseCase` è vero, con la stessa struttura del template — 1.0 Problema/opportunità di business · 1.1 Soluzione proposta · 1.2 Obiettivi strategici · 1.3 Dati e contesto · 1.4 Impatto atteso · 1.5 Metriche di successo · 1.6 Valutazione etica preliminare · 1.7 Rischi, complessità e resistenze — con le informazioni raccolte già organizzate nei campi.

- Nessun campo è modificabile: per correggere o aggiungere qualcosa si torna alla conversazione con **"Torna alla conversazione"**, si racconta il dettaglio, e si preme di nuovo "Procedi allo Use Case" quando si è pronti.
- Si conferma con **"Confermo la scheda"** (imposta `completedAt`); la bozza si autosalva come negli altri step.
...
- La dashboard del facilitatore mostra lo stato dello Step 4 per ogni partecipante (Incompleto · In compilazione · Pronto per lo Use Case · Completo) e un pulsante **PDF per ogni partecipante** che scarica la sua scheda dai dati salvati (non serve che la pagina del partecipante sia aperta).

## Modelli OpenAI

- **Step 1 e 2 (`CHAT_MODEL`, `gpt-5-nano`)**: assistenti di supporto — spiegano le domande, aiutano a tarare le risposte, non conducono un'intervista. Non serve reasoning pesante: è il modello più economico della gamma OpenAI corrente.
- **Step 4 (`USE_CASE_MODEL`, `gpt-5-mini`)**: conduce l'intervista, deve estrarre più campi da una risposta, riconoscere risposte insufficienti e l'intenzione di terminare, valutare quando l'intervista è ragionevolmente esaurita. Serve reasoning reale, non solo linguaggio fluente.
- Entrambi in `src/lib/openaiClient.ts`. I modelli della famiglia GPT-5 non supportano il parametro `temperature` (accettano solo il default): le chiamate in `api/agent/route.ts` non lo passano.
